...
export class BaseMpl {
  ...
  async createToken(input: CreateNftBuilderParams, opts: { decimal?: number, mintAmount?: number, mintKeypair?: web3.Keypair }) {
    const ixs = [];
    ...
    try {
      ...
      tx.feePayer = user;
      const res = await this.provider.sendAndConfirm(tx, [mintKeypair]);
      return {
        txSignature: res,
